...
jest.mock('@/components/ui/DragAndDrop', () => ({
    ...
    DnDDroppable: ({ children }: { children: (provided: unknown) => React.ReactNode }) => children({ innerRef: jest.fn(), droppableProps: {}, placeholder: null }),
    DnDDraggable: ({ children }: { children: (provided: unknown, snapshot: unknown) => React.ReactNode }) => children({ innerRef: jest.fn(), draggableProps: {}, dragHandleProps: {} }, { isDragging: false }),
}))
...
describe('KeyAchievements Component', () => {
    ...
    const renderComponent = (props = { workExperienceIndex: 0 }) => {
        return render(
            <ResumeContext.Provider value={{ resumeData: mockResumeData, setResumeData: jest.fn() } as unknown as React.ContextType<typeof ResumeContext>}>
    ...
    describe('AI Sorting', () => {
        ...
        it('does not sort if work experience is missing from context', async () => {
            const Wrapper = () => (
                <ResumeContext.Provider value={{ resumeData: { workExperience: [] }, setResumeData: jest.fn() } as unknown as React.ContextType<typeof ResumeContext>}>
    ...
    it('does not reorder if dropped outside (no destination)', () => {
        ...
        const calls = require('@/components/ui/DragAndDrop').DnDContext.mock.calls
