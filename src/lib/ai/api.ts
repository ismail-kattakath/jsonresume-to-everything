import { OpenAIRequest, OpenAIResponse, OpenAIError, OpenAIConfig } from '@/types/openai'
...
export function sanitizeAIError(error: unknown): string {
  ...
  if (message.includes('ServiceUnavailableError') || message.includes('503') || message.includes('Overloaded')) {
 ...
 */
export async function makeOpenAIRequest(config: OpenAIConfig, request: OpenAIRequest): Promise<OpenAIResponse> {
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        throw new AIAPIError(sanitizedMessage, errorData.error?.code, errorData.error?.type)
      } catch (parseError) {
    ...
    if (error instanceof Error) {
      if (error.name === 'AbortError') {
        throw new OpenAIAPIError('API request timed out. The server took too long to respond.', 'timeout')
      }
