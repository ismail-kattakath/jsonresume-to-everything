...
export const validateJDFormatTool = tool({
  ...
  callback: (input: { jd_text: string }) => {
    const requiredSections = ['position-title', 'core-responsibilities', 'desired-qualifications', 'required-skills']
    const issues: string[] = []
    ...
    const qualificationsMatch = input.jd_text.match(/# desired-qualifications([\s\S]*?)(?=# required-skills|$)/)
