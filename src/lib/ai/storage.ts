...
export async function saveCredentials(credentials: StoredCredentials): Promise<void> {
  ...
  const encryptedApiKey = credentials.apiKey ? await encryptData(credentials.apiKey, vaultKey) : ''

  const encryptedProviderKeys: Record<string, string> = {}
  if (credentials.providerKeys) {
    const entries = Object.entries(credentials.providerKeys)
    for (const [url, key] of entries) {
      // Use a safe way to set properties to avoid object injection warnings
      Object.defineProperty(encryptedProviderKeys, url, {
        value: await encryptData(key, vaultKey),
        enumerable: true,
        configurable: true,
        writable: true,
      })
  ...
  try {
    ...
    if (credentials.providerKeys) {
      ...
      for (const [url, key] of Object.entries(credentials.providerKeys)) {
        try {
          const decrypted = await decryptData(key, vaultKey)
          Object.defineProperty(decryptedKeys, url, {
            value: decrypted,
            enumerable: true,
            configurable: true,
            writable: true,
      ...
      credentials.providerKeys = decryptedKeys
    }
