...
import { AISettingsProvider, useAISettings } from '@/lib/contexts/AISettingsContext'
...
describe('AISettingsContext', () => {
  ...
  describe('Initialization', () => {
    ...
    it('loads saved credentials on mount', async () => {
      ...
      await waitFor(() => {
        expect(result.current.settings.apiUrl).toBe('https://openrouter.ai/api/v1')
        expect(result.current.settings.apiKey).toBe('saved-key')
        expect(result.current.settings.model).toBe('gpt-4o')
        expect(result.current.settings.jobDescription).toBe('Saved job description')
      })
    })
  ...
  describe('Job Description Validation', () => {
    ...
    it('skips re-validation for unchanged job description', async () => {
      ...
      // Explicitly check status hasn't changed without needing to wait for timer
