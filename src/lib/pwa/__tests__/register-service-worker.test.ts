/**
 * Tests for the Service Worker registration and unregistration logic.
...
describe('registerServiceWorker', () => {
  ...
  it('should register service worker on load event', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith('✅ Service Worker registered:', '/resume/builder')
  })
  ...
  it('should detect service worker updates', async () => {
    ...
    const statechangeHandler = mockInstalling.addEventListener.mock.calls.find((call) => call[0] === 'statechange')?.[1]

    mockInstalling.state = 'installed'
    statechangeHandler()

    expect(console.log).toHaveBeenCalledWith('🔄 New version available - reload to update')
  })
