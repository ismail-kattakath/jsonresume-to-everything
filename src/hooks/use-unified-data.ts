...
/**
 * A comprehensive hook for managing and synchronizing resume and cover letter data.
 * Handles persistence to localStorage and ensures shared fields remain in sync.
 */
export function useUnifiedData() {
    ...
    if (initialResumeData.skills && initialResumeData.skills.length > 0) {
      const needsMigration = initialResumeData.skills.some((skillCategory) =>
        skillCategory.skills.some(
          (skill) => typeof skill === 'string' || ('underline' in skill && skill.highlight === undefined)
        )
      )
    ...
    if (savedCLData) {
      try {
        ...
        setCoverLetterData({
          ...defaultResumeData,
          ...parsedData,
          content: parsedData.content && parsedData.content.trim() ? parsedData.content : DEFAULT_COVER_LETTER_CONTENT,
        })
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const isDefaultData =
      JSON.stringify({ ...coverLetterData, content: '' }) === JSON.stringify({ ...defaultResumeData, content: '' })
  ...
  useEffect(() => {
    ...
    const resumeChanged = Object.keys(currentResumeFields).some(
      (key) =>
        JSON.stringify(currentResumeFields[key as keyof typeof currentResumeFields]) !==
        JSON.stringify(lastSyncedResumeData.current[key as keyof typeof currentResumeFields])
    )
  ...
  useEffect(() => {
    ...
    const coverLetterChanged = Object.keys(currentCoverLetterFields).some(
      (key) =>
        JSON.stringify(currentCoverLetterFields[key as keyof typeof currentCoverLetterFields]) !==
        JSON.stringify(lastSyncedCoverLetterData.current[key as keyof typeof currentCoverLetterFields])
    )
