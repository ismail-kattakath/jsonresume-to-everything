...
describe('useSkillGroupsManagement', () => {
  ...
  describe('addGroup', () => {
    it('adds a new skill group', () => {
      ...
      const { result } = renderHook(() => useSkillGroupsManagement(), { wrapper: createWrapper(data, setData) })
      act(() => result.current.addGroup('DevOps'))
      expect(setData).toHaveBeenCalledWith(
        expect.objectContaining({
          skills: expect.arrayContaining([expect.objectContaining({ title: 'DevOps', skills: [] })]),
        })
      )
