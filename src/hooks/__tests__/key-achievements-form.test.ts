...
describe('useKeyAchievementsForm', () => {
    ...
    describe('handleChange', () => {
        it('updates text of an achievement at given index', () => {
            let data = makeResumeData()
            let setDataImpl: (fn: (prev: string[]) => string[]) => void
            const setData = jest.fn((fn) => {
            ...
            const { result } = renderHook(
                () => useKeyAchievementsForm(0),
                { wrapper: createWrapper(data, setDataImpl) }
            )
